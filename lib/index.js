...
class Avanza extends EventEmitter {
  ...
  _backoffCalc(actionName) {
    const now = Date.now()
    let schedDelay = 0
    if (now - this._backOffTimestamps[actionName] < MAX_BACKOFF_MS * 2) {
  ...
  _socketHandleMessage(data) {
    ...
    if (response.length && response[0]) {
      ...
      if (message.advice && message.advice.reconnect === 'handshake') {
        ...
        if (message.advice && message.advice.timeout > this._adviceTimeout) {
          this._adviceTimeout = message.advice.timeout
        }
  ...
  _storeSessionCredentials(securityToken, authenticationSession, pushSubscriptionId, customerId) {
    this._authenticated = true
    this._securityToken = securityToken
    this._authenticationSession = authenticationSession
    this._pushSubscriptionId = pushSubscriptionId
  ...
  */
  authenticate(credentials) {
    ...
    if (!credentials.password && !credentials.personnr) {
      return Promise.reject('Must provide either credentials.password or credentials.personnr.')
    }
    ...
    if (credentials.password) {
      return request({
        ...
        return this._storeSessionCredentials(response.headers['x-securitytoken'],
                                             response.body.authenticationSession,
                                             response.body.pushSubscriptionId,
                                             response.body.customerId)
      });
    } else {
      return mbankid.invoke_auth_chain(USER_AGENT, credentials).then((session_info) => {
        return this._storeSessionCredentials(session_info.aza_usertoken,
                                             session_info.cookies['csid'],
                                             session_info.kontooversikt.pushSubscriptionid,
                                             session_info.custinfo.customerId)
      });
    }
  }

export function invoke_auth_chain(user_agent, session_info) {
    ...
    // After auth_eid_login Chrome client also calls
    // https://www.avanza.se/_cqbe/authentication/post-login?_=1526990016660
    //                                                            ^-- milliseconds since 1970
    // The purpose of this call is right now unknown.
}
...
function httplog(level, header, cookies, data) {
    if (debug) {
        console.log("====================");
        console.log("headers:");
        console.log(header);
...
function bankid_authenticate(session_info) {
    return new Promise(
        (resolve, reject) => {
            ...
            req.write(post_data);
            req.end();
            httplog(0, options, session_info.cookies, post_data);
        }
    );
}
...
function kontooversikt(session_info) {
    return new Promise(
        (resolve, reject) => {
            ...
            set_request_cookies(req, session_info.cookies);
            req.end();
            httplog(0, options, session_info.cookies, {});
        }
    );
}
